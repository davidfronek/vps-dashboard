...
type DomainSettings = Pick<DomainGroup, "key" | "name" | "names" | "automaticSsl" | "forceHttps" | "wwwRedirect" | "ssl" | "deployment" | "environmentKeys">;
...
export default function Dashboard({ adminUsername, snapshot: initialSnapshot }: { adminUsername: string; snapshot: ServerSnapshot }) {
  ...
  const [activeTodoCount, setActiveTodoCount] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/todos", { cache: "no-store" })
      .then(async (response) => {
        ...
        if (active) setActiveTodoCount((result.todos ?? []).filter((todo) => !todo.completed).length);
      })
      .catch(() => {});
    return () => { active = false; };
  }, []);
    ...
    <div className={pendingOperation ? "dashboard-shell operation-pending" : "dashboard-shell"} aria-busy={pendingOperation}>
      ...
      <main className="workspace">
        ...
        <div className="page-content">
          {activeSection === "Nastavení" ? <SettingsView notify={notify} /> : activeSection === "Úkoly" ? <TodoView notify={notify} onActiveCountChange={setActiveTodoCount} /> : activeSection === "Databáze" ? <DatabaseView databases={snapshot.databases} onRefresh={() => router.refresh()} runJob={executeJob} /> : activeSection === "Uživatelé" ? <UsersView users={snapshot.users} onRefresh={() => router.refresh()} runJob={executeJob} /> : <>
